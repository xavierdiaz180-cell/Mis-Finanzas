...
async function testDebtsCrud() {
  ...
  let editRes = null;
  const resEdit = {
    json: (data) => { editRes = data; return this; },
  ...
  console.log('Edited debt balance in DB:', editedDebt.current_balance);

  // Test delete debt
  const reqDel = { params: { id: testDebtId } };
  let delRes = null;
  const resDel = {
    json: (data) => { delRes = data; return this; },
  ...
  if (!deletedCheck && editedDebt.current_balance === 6000) {
