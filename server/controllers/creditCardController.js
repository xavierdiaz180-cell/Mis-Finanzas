...
const { executeCardPayment, executeCardPurchase } = require('../services/transactionService');
 ...
 * Delegates to executeCardPayment (ACID) — single authoritative engine for card payments.
 * Includes automatic MSI installment advancement when payment >= monthly installment.
 */
async function payDebt(req, res) {
  try {
    ...
    if (!account_id || !amount || parseFloat(amount) <= 0) {
      return res.status(400).json({ error: 'Cuenta de origen y monto valido son requeridos.' });
    }
    ...
    if (!debt.account_id) {
      return res.status(400).json({ error: 'Esta deuda no tiene cuenta de tarjeta vinculada. Edita la deuda para vincular una cuenta.' });
    }

    const result = await executeCardPayment({
      source_account_id: parseInt(account_id, 10),
      credit_card_id:    parseInt(debt.account_id, 10),
      amount:            parseFloat(amount),
      concept:           'Pago a ' + debt.name,
    ...
    return res.json({
      ...
      msi_advanced:   result.msi_advanced,
      message:        `Pago de $${parseFloat(amount).toLocaleString('es-MX')} aplicado a ${debt.name}.`
    });
  } catch (error) {
 ...
 * Delegates to executeCardPurchase (ACID) — single authoritative engine for card purchases.
 * Supports MSI: creates installment plan automatically within the same ACID transaction.
 */
async function addCardExpense(req, res) {
  try {
    ...
    const result = await executeCardPurchase({
      credit_card_id: parseInt(debt.account_id, 10),
      amount:         parseFloat(amount),
      concept:        concept || `Gasto en ${debt.name}`,
      category,
      is_msi,
      msi_months:     parseInt(msi_months, 10),
    ...
    return res.json({
      success:        true,
      transaction_id: result.transaction_id,
      msi_plan:       result.msi_plan,
      message:        `Gasto de $${parseFloat(amount).toLocaleString('es-MX')} registrado en ${debt.name}.`
    });
  } catch (error) {
